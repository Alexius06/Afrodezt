...
export default function PartnerCarouselSection({ partnersInView, partnersRef }) {
  return (
    <section
      id="partners"
      ref={partnersRef}
      className={`w-full py-16 px-4 md:px-8 relative overflow-hidden transition-all duration-1000 ${partnersInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}
    >
  ...
  `}</style>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-navy/10 pb-6 gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <div className="flex items-center ml-1 mb-1 justify-center gap-3">
                <span className="h-[2px] w-8 bg-fire inline-block" />
                <p className="text-maroon text-xs font-mono font-light uppercase">
      ...
      <div className="relative z-10 w-full overflow-hidden py-4">
        ...
        <div className="absolute top-0 bottom-0 left-0 w-16 md:w-32 bg-gradient-to-r from-paper via-paper/80 to-transparent z-20 pointer-events-none" />

        {/* Right Edge Gradient Blur */}
        <div className="absolute top-0 bottom-0 right-0 w-16 md:w-32 bg-gradient-to-l from-paper via-paper/80 to-transparent z-20 pointer-events-none" />
        ...
        <div className="animate-partner-marquee gap-6">
          {CAROUSEL_ITEMS.map((partner, index) => (
            <div
              ...
              className="group relative w-[280px] md:w-[500px] shrink-0 bg-white/90 backdrop-blur-sm border border-navy/10 rounded-lg p-6 flex flex-col justify-between transition-all duration-300 hover:border-maroon/50 hover:shadow-lg hover:shadow-navy/5 hover:-translate-y-1 overflow-hidden"
            >
              ...
              <div>
                ...
                <p className="text-steel text-xs line-clamp-1">
                  {partner.domain}
                </p>
              </div>
            </div>
